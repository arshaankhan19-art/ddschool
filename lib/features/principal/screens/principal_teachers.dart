...
class _PrincipalTeachersScreenState extends State<PrincipalTeachersScreen> {
  ...
  final List<String> _filters = ['All', 'Active', 'Class Teachers', 'Subject Teachers'];
  ...
  Widget _buildEmptyState() {
    return Column(
      children: [
        const SizedBox(height: 40),
        Icon(Icons.person_off_rounded, size: 60, color: AppColors.textSecondary.withOpacity(0.3)),
        const SizedBox(height: 16),
        Text(
          'No teachers found',
          style: TextStyle(color: AppColors.textSecondary.withOpacity(0.5), fontWeight: FontWeight.bold),
        ),
      ],
    );
  }
    ...
    return Container(
      ...
      child: TextField(
        decoration: InputDecoration(
          hintText: 'Search teacher by name, subject or ID',
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          _buildStatCard('Active Now', active.toString(), Icons.how_to_reg_rounded, Colors.green),
          _buildStatCard('On Leave', leave.toString(), Icons.event_busy_rounded, Colors.orange),
          _buildStatCard('Pending Tasks', '7', Icons.assignment_rounded, Colors.redAccent),
        ],
      ),
    );
  }
            ...
            Column(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isOnLeave ? Colors.orange.withOpacity(0.1) : Colors.green.withOpacity(0.1),
                    borderRadius: BorderRadius.circular(20),
                  ),
                  child: Row(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildOptionTile(context, Icons.person_outline_rounded, 'View Full Profile', onTap: () {
              Navigator.pop(context);
  ...
  void _confirmRemoveTeacher(BuildContext context, Map<String, dynamic> teacher) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to remove ${teacher['name']} from active teachers of DD School?',
  ...
  void _showAddTeacherModal(BuildContext context) {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => const _TeacherFormModal(isEdit: false),
    );
  }
  ...
  void _showEditTeacherModal(BuildContext context, Map<String, dynamic> teacher) {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => _TeacherFormModal(isEdit: true, teacher: teacher),
    );
  }
}
...
class _TeacherDetailsModal extends StatelessWidget {
              ...
              child: Column(
                children: [
                  ...
                  _buildInfoSection('PROFESSIONAL DETAILS', [
                    ...
                    _buildInfoRow(Icons.payments_outlined, 'Salary (UI only)', '₹65,000'),
                    _buildInfoRow(Icons.supervised_user_circle_outlined, 'Class Teacher Of', teacher['class']),
                  ]),
                  const SizedBox(height: 24),
                  _buildInfoSection('WORK STATUS', [
                    _buildInfoRow(Icons.calendar_month_rounded, 'Attendance %', teacher['attendance']),
                    _buildInfoRow(Icons.play_circle_outline_rounded, 'Total Classes Taken', teacher['classesTaken']),
  ...
  Widget _buildActionsSection() {
    return Column(
      children: [
        _buildActionButton(Icons.calendar_month_rounded, 'View Timetable'),
        const SizedBox(height: 12),
        _buildActionButton(Icons.history_rounded, 'Attendance Record'),
        const SizedBox(height: 12),
        _buildActionButton(Icons.description_rounded, 'Performance Report'),
        const SizedBox(height: 12),
        _buildActionButton(Icons.phone_in_talk_outlined, 'Contact Teacher', isPrimary: true),
      ],
    );
  }
  ...
  Widget _buildActionButton(IconData icon, String label, {bool isPrimary = false}) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        onPressed: () {},
        icon: Icon(icon, size: 20, color: isPrimary ? Colors.white : AppColors.primary),
        label: Text(label),
        style: ElevatedButton.styleFrom(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 16),
                  _buildTextField('Mobile Number', Icons.phone_android_rounded, initialValue: '+91 98765 43210'),
                  const SizedBox(height: 16),
                  _buildTextField('Email', Icons.email_outlined, initialValue: 'teacher@ddschool.com'),
                  const SizedBox(height: 16),
                  _buildTextField('Subject', Icons.menu_book_rounded, initialValue: teacher?['subject']),
                  const SizedBox(height: 16),
                  _buildTextField('Department', Icons.account_tree_outlined, initialValue: 'Science'),
                  const SizedBox(height: 16),
                  _buildDropdownField('Designation', ['PGT', 'TGT', 'PRT'], initialValue: teacher?['designation']),
                  const SizedBox(height: 16),
                  _buildTextField('Qualification', Icons.school_outlined, initialValue: 'M.Sc, B.Ed'),
                  const SizedBox(height: 16),
                  _buildTextField('Experience', Icons.timeline_rounded, initialValue: '10 Years'),
                  const SizedBox(height: 16),
  ...
  Widget _buildUploadUI() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: const Color(0xFFEEEEEE), style: BorderStyle.none),
      ),
      child: Column(
        children: [
          Icon(Icons.cloud_upload_outlined, size: 40, color: AppColors.primary.withOpacity(0.5)),
          const SizedBox(height: 12),
